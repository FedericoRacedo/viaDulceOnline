...
async function cargarProductos() {
    try {
        const response = await fetch('../json/almacen.json'); // Verifica esta ruta
        if (!response.ok) {
...
function agregarFiltros(productos) {
    ...
    document.getElementById('filtro-yerbas').addEventListener('click', () => {
        filtrarPorCategoria(productos, 'Yerbas');
    });
    ...
    document.getElementById('filtro-cocina').addEventListener('click', () => {
        filtrarPorCategoria(productos, 'Cocina')
    });
